...
public class ProductValidator {
    public static void validateProductName(String productName) {
        if (productName.length() > Constant.MAX_PRODUCT_NAME_LENGTH.getValue()) {
            throw new LengthErrorException("%d자를 초과할 수 없습니다.".formatted(Constant.MAX_PRODUCT_NAME_LENGTH.getValue()));
        }
    }

    public static void validateDescription(String description) {
        if (description != null && description.length() > Constant.MAX_PRODUCT_DESCRIPTION_LENGTH.getValue()) {
            throw new LengthErrorException("%d자를 초과할 수 없습니다.".formatted(Constant.MAX_PRODUCT_DESCRIPTION_LENGTH.getValue()));
        }
    }

    public static void validatePrice(long price) {
        if (price > Constant.MAX_PRODUCT_PRICE.getValue() || price < Constant.MIN_PRODUCT_PRICE.getValue()) {
